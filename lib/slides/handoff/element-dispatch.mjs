...
// SVG gradient entry. The paint encodes position via a 2x3 affine `transform`
// that maps the shape's 0..1 local space onto Figma's unit-square gradient
// coordinates — see node_modules/openfig-core/dist/index.cjs resolveGradient-
// Geometry(). For linear gradients this means any pair of endpoints (x1,y1)→
// (x2,y2) in shape-local 0..1 coords can be represented.
function buildGradientPaint(g) {
  ...
  if (g.type === 'linear') {
    ...
    // T maps shape-local (x, y) in 0..1 to Figma unit-square (u, v) such
    // that (x1,y1) → (0, 0.5) and (x2,y2) → (1, 0.5).
    const m00 = dx / det;
    const m01 = dy / det;
  ...
  // Radial: center (cx, cy), radius r in shape-local 0..1 coords. The mapping
  // (cx, cy) → (0.5, 0.5), (cx + r, cy) → (1, 0.5) gives the right geometry.
  const { cx, cy, r } = g;
  if (!r) return null;
  const m00 = 1 / r / 2;
  const m02 = 0.5 - cx / r / 2;
  const m11 = 1 / r / 2;
  const m12 = 0.5 - cy / r / 2;
  return {
...
// Returns a Map<id, { type, x1, y1, x2, y2, cx, cy, r, stops }>.
function parseSvgGradients(markup) {
  ...
  while ((gm = gradRe.exec(markup)) !== null) {
    ...
    const entry = { type: kind === 'linearGradient' ? 'linear' : 'radial', stops };
    if (entry.type === 'linear') {
      entry.x1 = numAttr(attrsChunk, 'x1') ?? 0;
      entry.y1 = numAttr(attrsChunk, 'y1') ?? 0;
      entry.x2 = numAttr(attrsChunk, 'x2') ?? 1;
...
async function handleSvg(slide, el, ctx) {
  ...
  const resolveFill = (raw) => {
    ...
    return { gradient: buildGradientPaint(g) };
  };
  ...
  for (const sh of shapes) {
    if (sh.type === 'circle') {
      const { fill, gradient } = resolveFill(sh.fill);
