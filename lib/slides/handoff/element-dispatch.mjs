...
function transformPathD(d, X, Y) {
  return d.replace(/([MLQCTSHVAZ])\s*((?:[-+]?\d*\.?\d+\s*,?\s*)*)/gi, (_full, cmd, nums) => {
    const parts = nums.trim().split(/[\s,]+/).filter(Boolean).map(Number);
    const upper = cmd.toUpperCase();
    if (upper === 'Z') return 'Z';
    const out = [];
    if (upper === 'H') {
      for (const n of parts) out.push(X(n).toFixed(3));
      return `H ${out.join(' ')}`;
    }
    if (upper === 'V') {
      for (const n of parts) out.push(Y(n).toFixed(3));
      return `V ${out.join(' ')}`;
    }
    for (let i = 0; i < parts.length; i += 2) {
      out.push(`${X(parts[i]).toFixed(3)} ${Y(parts[i + 1]).toFixed(3)}`);
    }
    return `${upper} ${out.join(' ')}`;
  });
}
  ...
  for (const sh of shapes) {
    if (sh.type === 'circle') {
      ...
      slide.addPath(d, {
        ...
        strokeCap: (sh.strokeLinecap ?? 'butt').toUpperCase(),
      });
