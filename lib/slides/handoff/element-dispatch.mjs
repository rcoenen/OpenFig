...
function textOpts(el) {
  ...
  if (el.noWrap) {
    ...
    // Multi-line text: let the Figma node's height track the rendered
    // content so it isn't frozen to Chromium's measurement. Figma's text
    // layout may produce a different line count than Chromium did; an
    // auto-resizing node adapts rather than visually overflowing a fixed
    // box. See openspec/changes/fix-html-converter-figma-fidelity Phase 1.
...
async function handleSvg(slide, el, ctx) {
  ...
  const applyOpacity = (node) => { if (svgOpacity != null && node) node.opacity = svgOpacity; };
