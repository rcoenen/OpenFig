...
// Figma's Inter glyph advances run ~20–25% wider than Chromium's at the
// same size/weight (observed empirically, e.g. Chromium "11" at fontSize
// 420 with letter-spacing -0.03em measures 316px, Figma renders ~380px).
// Use 1.3× as a safety buffer when we need to predict Figma's width.
const FIGMA_INTER_WIDTH_BUFFER = 1.3;
...
function textOpts(el, ctx = {}) {
  ...
  // - Multi-line noWrap (titles split by explicit \n): HEIGHT. The browser-
  //   measured height (e.g. 275) reflects Chrome's rendering, but Figma
  //   positions glyphs differently when lineHeight < fontSize — descenders
  //   overflow the browser-measured height and dip into the next absolute
  //   sibling. Letting Figma auto-grow the frame fits its own descender
  //   placement; siblings have enough buffer in practice.
  // - Wrapping text: HEIGHT so Figma can reflow vertically.
  const isMultiLineNoWrap = el.noWrap
    && typeof el.text === 'string'
    && el.text.includes('\n');
  if (el.noWrap && !isMultiLineNoWrap) {
    opts.autoresize = 'WIDTH_AND_HEIGHT';
    // Figma Slides enforces an implicit wrap boundary at
    // (slide_right_edge − text_x), even for textAutoResize:
    // WIDTH_AND_HEIGHT. Setting size.x = 16384 does NOT override this —
    // the slide-edge boundary wins. This is Slides-specific: open-
    // pencil's reference Figma text layout (canvas/text.ts
    // resolveParagraphLayoutWidth) returns 1e6 for WIDTH_AND_HEIGHT
    // nodes, so regular Figma Design never wraps them; Slides must be
    // passing (slide_right − text_x) as maxWidth because the SLIDE
    // node is a bounded 1920×1080 clipping container.
    //
    // Because Figma's Inter is wider than Chromium's Inter, the
    // Chromium-measured x+width may sit just barely inside the slide
    // but Figma's widened glyphs overflow the boundary, causing
    // single-word text (e.g. a large "11" section number placed via
    // CSS right:56px) to wrap vertically. If our predicted Figma width
    // doesn't fit in the available slide space, shift x leftward to
    // give Figma room to lay out on one line. Visual position can
    // drift ≤ a few dozen pixels left — acceptable for the typical
    // right-anchored-big-number case this protects.
    const slideWidth = ctx.slideWidth ?? 1920;
    const predictedFigmaWidth = (el.width ?? 0) * FIGMA_INTER_WIDTH_BUFFER;
    const available = slideWidth - el.x;
    if (predictedFigmaWidth > available) {
      const shift = predictedFigmaWidth - available;
...
async function handleStatWithRing(slide, el, ctx) {
  const boost = Math.round(el.label.lineHeight ?? 0);
...
async function handleSvg(slide, el, ctx) {
  ...
  for (const sh of shapes) {
    if (sh.type === 'circle') {
      ...
      const node = slide.addLine(X(sh.x1), Y(sh.y1), X(sh.x2), Y(sh.y2), lineOpts);
      applyOpacity(node, sh);
