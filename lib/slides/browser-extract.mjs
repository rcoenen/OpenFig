  ...
  return await page.evaluate(() => {
    ...
    function collectSection(root, index) {
      ...
      function emitInlineRuns(el, parentCs) {
        const parentLines = countTextLines(el);
        if (parentLines > 1) {
        ...
        for (const c of el.childNodes) {
          if (c.nodeType === 3) {
            const txt = collapseText(c.textContent);
            if (!txt) continue;
            const range = document.createRange();
            range.selectNode(c);
            const rects = range.getClientRects();
            ...
            for (const rc of rects) {
              ...
              tops.add(Math.round(rc.top));
            }
            const el2 = {
              type: 'text',
              text: txt,
              x: l - off.x, y: t - off.y, width: r - l, height: b - t,
              ...textStyle(parentCs),
            };
            if (tops.size === 1) el2.noWrap = true;
            elements.push(el2);
          } else if (c.nodeType === 1) {
            const ct = c.tagName.toUpperCase();
            if (ct === 'BR') continue;
            ...
            const cRect = toLocal(c.getBoundingClientRect());
            const txt = collapseText(c.innerText ?? c.textContent ?? '');
            if (!txt) continue;
            const lines = countTextLines(c);
            const el2 = {
              type: 'text',
              text: txt,
      ...
      function maybeEmitShape(el, cs, rect) {
        ...
        if (hasBg) {
          elements.push({
            ...
            x: rect.x, y: rect.y, width: rect.width, height: rect.height,
            fill: bg,
            ...(radius > 0 && !isEllipse ? { borderRadius: radius } : {}),
          });
        }
