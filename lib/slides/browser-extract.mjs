    ...
    function collectSection(root, index) {
      ...
      function emitTextLeaf(el, cs, rect) {
        ...
        if (lines === 1 && !style.align) {
          ...
          // If a segment between authored breaks ALSO soft-wraps inside its
          // box (more visual lines than \n-delimited segments), the box width
          // is load-bearing — e.g. "PARAMETER CLARITY" wrapping under
          // "FINDING 03" in a 200px label column. Leaving noWrap unset lets
          // Figma re-wrap within the measured width and reproduce that layout.
          // Forcing noWrap here would overflow the column into the next one.
          const visualLines = new Set(rangeRects.map((r) => Math.round(r.top))).size;
          const authoredLines = text.split('\n').length;
          if (visualLines <= authoredLines) {
            let widest = 0;
