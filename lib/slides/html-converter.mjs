...
function nonZeroBorder(style) {
  const b = parseBorder(style);
  if (!b) return null;
  const color = b.color && b.color !== 'transparent' ? b.color : null;
...
function borderEdges(style) {
  const edges = [];
  for (const side of ['top', 'right', 'bottom', 'left']) {
    const shorthand = style[`border-${side}`];
    if (shorthand && !/^(0|none|transparent)/i.test(shorthand)) edges.push(side);
  }
  if (style.border && !/^(0|none|transparent)/i.test(style.border)) edges.push('all');
  return edges;
}
...
function walkSection(section, rules, mediaMap, slideIdx, warn) {
  ...
  function visit(el, ancestry, flowParent) {
    ...
    const edges = borderEdges(style);
    if (edges.length) {
      warn(slideIdx, `border-${edges[0] === 'all' ? '' : edges.join('/')} — dropped (borders not rendered on flow containers)`, elPath(el));
    }
    ...
    const txt = elementIsTextOnly(el) ? flattenText(el) : '';
    const textEstWidth = txt ? estimateTextWidth(style, txt) : null;
    ...
    if (elementIsTextOnly(el)) {
      if (txt && (style['font-size'] || flowParent)) {
        const childStyles = resolveChildSpanStyles(el, rules, ancestry);
        if (childStyles.length >= 2) {
          const sizes = new Set(childStyles.map(s => s['font-size']).filter(Boolean));
          const colors = new Set(childStyles.map(s => s.color).filter(Boolean));
          const weights = new Set(childStyles.map(s => s['font-weight']).filter(Boolean));
          const divergent = [];
          if (sizes.size > 1) divergent.push(`font-size {${[...sizes].join(', ')}}`);
          if (colors.size > 1) divergent.push(`color {${[...colors].join(', ')}}`);
          if (weights.size > 1) divergent.push(`font-weight {${[...weights].join(', ')}}`);
          if (divergent.length) {
            warn(slideIdx, `merged ${childStyles.length} inline children with divergent ${divergent.join(' + ')} into one text run — styling collapsed`, `${elPath(el)} → "${txt.slice(0, 60)}${txt.length > 60 ? '…' : ''}"`);
          }
        }
        if (!style['font-size'] && childStyles.some(s => s['font-size'])) {
          warn(slideIdx, `container has no font-size but child spans do — merged text uses default 16px height (rows may overlap)`, elPath(el));
        }
