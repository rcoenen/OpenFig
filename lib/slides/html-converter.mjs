...
export async function convertStandaloneHtml(htmlPath, outDeckPath, opts = {}) {
  ...
  // Surface any <img> CSS filters we recognised the shape of but don't bake
  // into the raster yet (anything outside parseImageFilter's allowlist).
  // Strips the diagnostic field after reporting so it never reaches the
  // handoff stage.
  for (const slide of manifestOut.slides) {
    for (const el of slide.elements ?? []) {
      if (el.type === 'image' && el.unhandledFilter) {
        collector.warn(
          slide.index - 1,
          `<img> filter not baked into raster: ${el.unhandledFilter} (only invert(1) and brightness(0) invert(1) are supported today)`,
          el.src,
        );
