...
function collectCssRules(doc, warn) {
  ...
  for (const r of rules) {
    if (/::?(first-line|first-letter|marker|placeholder)\b/.test(r.selector)) {
  ...
  if (warn) {
    ...
    for (const sel of pseudoSelectors) {
      warn(-1, `pseudo-element rule "${sel}" — not rendered`);
    }
  }
  return rules;
}
...
function selectorMatches(el, sel, ancestry) {
  if (/::?(before|after|first-line|first-letter|marker|placeholder)\b/.test(sel)) return false;
...
function resolvePseudoStyle(el, rules, ancestry, pseudo) {
  const re = new RegExp(`::?${pseudo}$`);
  const matched = [];
  for (const r of rules) {
    const m = r.selector.match(re);
    if (!m) continue;
    const base = r.selector.slice(0, m.index).trim();
    if (!base) continue;
    if (selectorMatches(el, base, ancestry)) {
      matched.push({ spec: specificity(r.selector), decls: r.declarations, order: matched.length });
    }
  }
...
function parsePseudoContent(raw) {
  ...
  if (s === 'none' || s === 'normal') return null;
  const q = s.match(/^(['"])(.*)\1$/);
  if (q) return q[2];
  if (/^(attr|counter|counters|url|var)\s*\(/.test(s)) return null;
  return s;
}

function emitPseudo(pStyle, parentBox, elements) {
  if (!pStyle) return;
  const content = parsePseudoContent(pStyle.content);
  if (content == null) return;
  const px0 = (v) => { const n = px(v); return Number.isFinite(n) ? n : 0; };
  const parentX = parentBox.x ?? 0;
  const parentY = parentBox.y ?? 0;
  const left = px0(pStyle.left);
  const top = px0(pStyle.top);
  const w = px(pStyle.width);
  const h = px(pStyle.height);
  const box = { x: parentX + left, y: parentY + top, width: w, height: h };
  if (content === '') {
    if (box.width == null || box.height == null) return;
    const shape = isEllipseDiv(pStyle, box) ? ellipseElement(pStyle, box) : rectElement(pStyle, box);
    if (shape) elements.push(shape);
    return;
  }
  const size = px(pStyle['font-size']) ?? 16;
  const out = {
    type: 'text',
    text: content,
    x: box.x, y: box.y,
    width: box.width ?? Math.ceil(content.length * size * 0.6),
    font: pStyle['font-family'],
    size,
    weight: fontWeightNum(pStyle['font-weight']),
    color: parseColor(pStyle.color),
  };
...
function walkSection(section, rules, mediaMap, slideIdx, warn) {
  ...
  function visit(el, ancestry, flowParent) {
    ...
    const beforePS = resolvePseudoStyle(el, rules, ancestry, 'before');
    const afterPS = resolvePseudoStyle(el, rules, ancestry, 'after');
    if (beforePS) emitPseudo(beforePS, box, elements);
    if (afterPS) emitPseudo(afterPS, box, elements);
