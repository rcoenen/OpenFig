...
function renderVector(deck, node) {
  ...
  // Fill paths — per-path fills via vectorData.styleOverrideTable when present.
  // Each fillGeometry entry may carry a styleID that maps to a different fill color.
  // Paths are grouped by effective fill color, then combined into compound paths so
  ...
  if (node.fillGeometry?.length && blobs) {
    ...
    // Group fillGeometry entries by effective fill color
    ...
    for (const geo of node.fillGeometry) {
      const color = (geo.styleID && styleMap.has(geo.styleID))
        ...
        : fillColor;
      if (!color) continue;
      if (!fillGroups.has(color)) fillGroups.set(color, { segs: [], hasEvenOdd: false });
      const group = fillGroups.get(color);
      const d = decodeCmdBlob(blobs, geo.commandsBlob);
      if (d) group.segs.push(d);
      if (geo.windingRule === 'EVENODD') group.hasEvenOdd = true;
    }

    for (const [color, { segs, hasEvenOdd }] of fillGroups) {
      if (!segs.length) continue;
      const rule = (segs.length > 1 || hasEvenOdd) ? ' fill-rule="evenodd"' : '';
      parts.push(`<path d="${segs.join('')}" fill="${color}"${rule}/>`);
    }
  }
...
const RENDERERS = {
  ...
  GROUP:             renderGroup,
  SECTION:           renderGroup,
  BOOLEAN_OPERATION: renderVector,
  VECTOR:            renderVector,
  LINE:              renderLine,
...
export function frameToSvg(fig, node) {
  ...
  const bg = resolveFill(getFillPaints(node)) ?? 'white';
  ...
  if (node.frameMaskDisabled === true) {
    ...
    vx = Math.floor(bounds.minX);
    vy = Math.floor(bounds.minY);
    w = Math.ceil(bounds.maxX) - vx;
    h = Math.ceil(bounds.maxY) - vy;
  }

  return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" \
width="${w}" height="${h}" viewBox="${vx} ${vy} ${w} ${h}">
  <rect x="0" y="0" width="${fw}" height="${fh}" fill="${bg}"/>
${body}
</svg>`;
}
