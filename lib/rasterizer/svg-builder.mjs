...
function renderEllipse(deck, node) {
  const { x, y } = pos(node);
  const { w, h } = size(node);
  const fill = resolveFill(node.fillPaints) ?? 'none';
  const stroke = strokeAttrs(node);
...
function renderText(deck, node) {
  ...
  // TODO: extract per-character style runs from textData.styleOverrideTable
  const fontSize = node.textData?.fontSize ?? node.style?.fontSize ?? 24;
  const fontFamily = node.textData?.fontFamily ?? node.style?.fontFamily ?? 'Inter';
  const fill = resolveFill(node.fillPaints) ?? '#000000';
  ...
  // SVG <text> doesn't wrap — clip to bounding box via clipPath
  const clipId = `tc-${nid(node)?.replace(':', '-')}`;
  return [
    ...
    `  fill="${fill}" clip-path="url(#${clipId})">${escaped}</text>`,
  ].join('\n');
}
